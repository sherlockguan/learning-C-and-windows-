...
BEGIN_MESSAGE_MAP(CNotepadPlusDlg, CDialogEx)
	...
	ON_COMMAND(IDM_UNDO, &CNotepadPlusDlg::OnUndo)
	ON_COMMAND(IDM_REPLACE_SEL, &CNotepadPlusDlg::OnReplaceSel)
END_MESSAGE_MAP()
...
BOOL CNotepadPlusDlg::OnInitDialog()
{
	...
	HWND hEdit = ::GetDlgItem(m_hWnd, IDC_EDIT2);
	m_edit.Attach(hEdit);
...
void CNotepadPlusDlg::OnUndo()
{
	m_edit.Undo();
}


void CNotepadPlusDlg::OnReplaceSel()
{
	//获取文本的长度
	int nLength = m_edit.GetWindowTextLength();
	m_edit.SetSel(nLength / 2, nLength / 2);

	m_edit.ReplaceSel(L"Hello World");
}
